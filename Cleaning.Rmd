...
.9621-crosscorr^2
```
###BEGIN MODEL FOR TOTAL
```{r}
#Create dataset excluding some observations so that cross-validation is possible
Total_holdout <- subset(GAMES, YEAR == 2003)
GAMES_total <- subset(GAMES, YEAR >= 2004)


#Create a model with all variables
total_all <- lm(log(Total) ~ HOME_TEAM_ID + VISITOR_TEAM_ID + FG_PCT_home +FT_PCT_home+FG3_PCT_home+AST_home+REB_home+PTS_away+FG_PCT_away+FT_PCT_away+FG3_PCT_away+AST_away+REB_away+HOME_TEAM_WINS+OREB_home+OREB_away+OREB+DREB_home+DREB_away+DREB+OREBP_home+OREBP_away+DREBP_home+DREBP_away+EFGP_home+EFGP_away+TOVP_home+TOVP_away+FTR_home+FTR_away+HEIGHT_PROP_home+HEIGHT_PROP_away+AVG_HEIGHT_home+AVG_HEIGHT_away+AVG_WEIGHT_home+AVG_WEIGHT_away+OREB_diff+DREB_diff+OREBP_diff+DREBP_diff+EFGP_diff+TOVP_diff+FTR_diff+HEIGHT_PROP_diff+WEIGHT_PROP_diff+AVG_HEIGHT_diff+AVG_WEIGHT_diff, data = GAMES_total)

summary(total_all)
```
```{r}
#Stepwise function to find the best variables
MSE = (summary(total_all)$sigma)^2
step(total_all, scale = MSE)
```
```{r}
best_total <- lm(formula = log(Total) ~ FT_PCT_home + FG3_PCT_home + AST_home + 
    REB_home + PTS_away + FG_PCT_away + FT_PCT_away + FG3_PCT_away + 
    AST_away + HOME_TEAM_WINS + OREB_home + OREB_away + DREB_away + 
    OREBP_home + EFGP_home + EFGP_away + TOVP_home + TOVP_away + 
    FTR_home + FTR_away + HEIGHT_PROP_home + AVG_HEIGHT_home + 
    AVG_HEIGHT_away + AVG_WEIGHT_away, data = GAMES_total)
summary(best_total)
```
```{r}
best_total2 <- lm(formula = log(Total) ~ FT_PCT_home + FG3_PCT_home + AST_home + 
    REB_home + PTS_away + FG_PCT_away + FT_PCT_away + FG3_PCT_away + HOME_TEAM_WINS + OREB_home + OREB_away + DREB_away + 
    OREBP_home + EFGP_home + EFGP_away + TOVP_home + TOVP_away + 
    FTR_home + FTR_away + HEIGHT_PROP_home + 
    AVG_HEIGHT_away + AVG_WEIGHT_away, data = GAMES_total)
...
```
```{r}
#Correlations to add interactions
total_cor <- correlate(GAMES_total[sapply(GAMES_total, is.numeric)])
total_cor %>% 
  focus(FT_PCT_home,FG3_PCT_home,AST_home,REB_home, PTS_away,FG_PCT_away,FT_PCT_away,FG3_PCT_away,HOME_TEAM_WINS,OREB_home,OREB_away,DREB_away,OREBP_home, EFGP_home, EFGP_away, TOVP_home,TOVP_away,FTR_home, FTR_away, HEIGHT_PROP_home, AVG_HEIGHT_away, AVG_WEIGHT_away, mirror=TRUE)
```
```{r}
best_total3 <- lm(formula = log(Total) ~ FT_PCT_home + FG3_PCT_home + AST_home + 
    REB_home + PTS_away + FG_PCT_away + FT_PCT_away + FG3_PCT_away + HOME_TEAM_WINS + OREB_home + OREB_away + DREB_away + 
    OREBP_home + EFGP_home + EFGP_away + TOVP_home + TOVP_away + 
    FTR_home + FTR_away + HEIGHT_PROP_home + 
    AVG_HEIGHT_away + AVG_WEIGHT_away +(PTS_away*FG_PCT_away)+(OREBP_home*OREB_home)+(PTS_away*EFGP_away)+(FG_PCT_away*EFGP_away), data = GAMES_total)
summary(best_total3)
anova(best_total2, best_total3)
```
```{r}
#Cross-Validation
PredictTotal <- predict(best_total3, newdata=Total_holdout)
holdoutresid_total = log(Total_holdout$Total) - (PredictTotal)
hist(holdoutresid_total)
mean(holdoutresid_total)
crosscorr_total=cor(log(Total_holdout$Total), PredictTotal)
crosscorr_total^2
#Getting the shrinkage
.9834-crosscorr_total
```
